  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">Monitor rate limits and configure fallback routing logic.</p>
      </div>
          ...
          return (
            <div key={model.id} className={`bg-surface border rounded-xl p-5 shadow-sm transition-colors ${isWarning ? 'border-yellow-500/50 bg-yellow-500/5' : 'border-border'}`}>
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-md ${isWarning ? 'bg-yellow-500/20 text-yellow-600' : 'bg-green-500/10 text-green-500'}`}>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-muted-foreground font-medium">Daily Tokens / Requests</span>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-muted-foreground font-medium">Requests Per Minute (RPM)</span>
                    <span className="text-foreground font-mono">{model.rpm} / {model.rpmLimit}</span>
                  </div>
                  <div className="w-full bg-background rounded-full h-2 overflow-hidden border border-border">
                    <div 
                      className={`h-full rounded-full ${rpmPercent > 90 ? 'bg-red-500' : rpmPercent > 75 ? 'bg-yellow-500' : 'bg-emerald-500'}`}

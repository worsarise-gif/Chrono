...
export const PlanetLogo = ({ className = "", showText = true }: { className?: string, showText?: boolean }) => (
  <div className={`font-bold text-xl tracking-tight text-black dark:text-white flex items-center justify-center select-none ${className}`}>
    <div className={`flex -rotate-45 items-center shrink-0 ${showText ? 'mr-1.5' : ''}`}>
      <svg width={showText ? "22" : "28"} height={showText ? "22" : "28"} viewBox="0 0 100 100" className="fill-current">

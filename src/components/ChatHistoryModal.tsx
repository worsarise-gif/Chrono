          ...
          >
            ...
            <div className="p-6 md:p-8 border-b border-border flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-foreground/5 flex items-center justify-center text-black dark:text-white">
                  <MessageSquare size={20} />
                </div>
                <div>
                  <h2 className="text-xl font-bold text-black dark:text-white">Chat History</h2>
                  <p className="text-sm text-black/60 dark:text-white/60">{chats.length} conversations found</p>
                </div>
              </div>
              <button
                onClick={onClose}
                className="p-2 rounded-full hover:bg-surface-hover text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white transition-all"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className="px-6 md:px-8 py-4 bg-surface/50 border-b border-border shrink-0">
              <div className="relative group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-black/40 dark:text-white/40 group-focus-within:text-black transition-colors" size={18} />
                <input
                  ref={searchInputRef}
                  type="text"
                  ...
                  className="w-full bg-surface-hover border border-border rounded-2xl pl-12 pr-4 py-3 text-black dark:text-white placeholder:text-black/40 focus:outline-none focus:ring-2 focus:ring-foreground/10 transition-all"
                />
              </div>
            </div>
                    ...
                    >
                      <div className="flex flex-col h-full justify-between space-y-4">
                        <div className="space-y-2">
                            ...
                            {editingChatId === chat.id ? (
                              <div className="flex-1 flex items-center gap-1 bg-surface border border-blue-500/50 rounded-lg p-1" onClick={(e) => e.stopPropagation()}>
                                <input
                                  ...
                                  className="flex-1 bg-transparent border-none outline-none text-sm text-black dark:text-white py-1 px-2"
                                />
                                <button 
                              ...
                              <>
                                <h3 className="font-semibold text-black dark:text-white line-clamp-2 leading-tight flex items-center gap-2">
                                  {chat.title}
                                  {chat.isPinned && (
                          ...
                          <div className="flex items-center gap-3 text-[11px] text-black/40 dark:text-white/40 font-medium uppercase tracking-wider">
                        ...
                        <div className="flex items-center justify-between pt-2">
                          <div className="flex items-center gap-1">
                            {editingChatId !== chat.id && (
                              <>
                                <button
                                  ...
                                  className={`p-2 rounded-lg transition-all ${
                                    chat.isPinned 
                                      ...
                                      : 'hover:bg-foreground/10 text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white'
                                  }`}
                                ...
                                <button
                                  ...
                                  className="p-2 rounded-lg hover:bg-foreground/10 text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white transition-all"
                                  title="Rename"
                                >
                                  <Edit2 size={14} />
                                </button>
                            ...
                            <button
                              ...
                              className="p-2 rounded-lg hover:bg-red-500/10 text-black/40 dark:text-white/40 hover:text-red-500 transition-all"
                              title="Delete"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
                          <div className="text-black/0 dark:text-white/0 group-hover:text-black/40 transition-all">
                            <ArrowRight size={16} />
                          </div>
                        </div>
                      </div>
            ...
            <div className="p-6 bg-surface/50 border-t border-border flex items-center justify-between shrink-0">
              <div className="text-xs text-black/40 dark:text-white/40 font-medium">

...
const NavItem = ({ icon, label, onClick, active, hasDot, isCollapsed, index }: any) => {
  return (
    <li className="w-full relative group">
      <button
        onClick={onClick}
        className={`flex items-center w-full rounded-lg transition-all relative pointer-events-auto h-[44px] ${active ? 'text-foreground dark:text-white' : 'text-foreground/60 dark:text-white/60 hover:text-foreground dark:hover:text-white'} ${!isCollapsed && !active ? 'hover:bg-surface' : ''}`}
      >
      ...
      {isCollapsed && (
        <div 
          className="fixed left-[76px] bg-surface-hover text-foreground dark:text-white text-[11px] px-[10px] py-[6px] rounded-[6px] opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-[9999] font-medium shadow-2xl transition-all duration-200 ease-in-out border border-border ml-[-10px] group-hover:ml-0"
          style={{ 
      ...
      <div className={`h-[100dvh] bg-sidebar-bg text-foreground border-r border-border z-50 font-sans transition-all duration-300 ease-in-out fixed md:relative ${isCollapsed ? 'w-[68px] overflow-visible' : 'w-[250px] overflow-x-hidden'} ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex items-center pt-5 pb-4 h-[60px] relative">
          <div className="w-[68px] flex-shrink-0 flex items-center justify-center">
            <PlanetLogo className="text-foreground dark:text-white w-6 h-6" />
          </div>
          <div className={`flex-1 flex items-center justify-end pr-4 transition-opacity duration-300 ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            <button
              ...
              className="text-foreground/60 dark:text-white/60 hover:text-foreground dark:hover:text-white transition-colors p-1 rounded-md hover:bg-surface hidden md:block"
            >
          ...
          <div className={`px-2 pb-2 flex flex-col min-h-full transition-opacity duration-300 ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            {isLoadingChats ? (
              ...
              <>
                  ...
                  {chats.map(chat => (
                    <li key={chat.id} className="group relative">
                      <button
                        ...
                        className={`w-full text-left block px-3 py-2 rounded-lg transition-colors text-[13px] font-normal truncate pr-8 ${currentChatId === chat.id ? 'text-foreground dark:text-white bg-surface' : 'text-foreground/60 dark:text-white/60 hover:bg-surface hover:text-foreground dark:hover:text-white'}`}
                      >
                        {chat.title}
                      </button>
                      ...
                      <div className="absolute left-0 top-0 w-full h-full pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-[50]">
                        <div className="absolute left-0 top-0 min-w-full bg-surface border border-border rounded-lg px-3 py-2 text-[13px] text-foreground dark:text-white shadow-xl whitespace-normal break-words z-[51]">
                          {chat.title}
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-md opacity-100 md:opacity-0 md:group-hover:opacity-100 hover:bg-surface-hover text-foreground/60 dark:text-white/60 hover:text-foreground dark:hover:text-white transition-all z-[60]"
                        title="More options"
                      >
                ...
                <button className="text-left px-3 py-2 text-[13px] text-foreground/40 dark:text-white/40 hover:text-foreground dark:hover:text-white mt-2 font-normal">
                  See all
                </button>
              </>
            )}
        ...
        <div className="mt-auto relative pb-4 pt-2">
          ...
          <div className={`absolute bottom-[112px] left-0 w-[68px] flex justify-center transition-opacity duration-300 hidden md:flex group ${isCollapsed ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button
              ...
              className="text-foreground/60 dark:text-white/60 hover:text-foreground dark:hover:text-white transition-colors p-2 rounded-full hover:bg-surface pointer-events-auto"
            >
            ...
            >
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold text-foreground dark:text-white">Delete Chat?</h3>
                  <p className="text-sm text-foreground/60 dark:text-white/60 leading-relaxed">
                    This will permanently delete <span className="text-foreground dark:text-white font-medium">"{chatToDelete.title}"</span> and all its messages. This action cannot be undone.
                  </p>
                </div>
                <div className="flex flex-col w-full gap-2 pt-2">
                  ...
                  <button
                    onClick={() => setChatToDelete(null)}
                    className="w-full py-3 px-4 bg-transparent hover:bg-surface text-foreground/60 dark:text-white/60 hover:text-foreground dark:hover:text-white rounded-xl font-medium transition-colors"
                  >
                    Cancel
                  </button>
                </div>

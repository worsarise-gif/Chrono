...
const NavItem = ({ icon, label, onClick, active, hasDot, isCollapsed }: any) => {
  return (
    <li className="w-full">
      <button
        onClick={onClick}
        className={`flex items-center gap-3.5 px-3 py-2.5 w-full rounded-lg transition-colors group relative ${active ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-[#1a1a1a]'}`}
      >
        <div className="relative flex-shrink-0 flex items-center justify-center w-[18px] h-[18px]">
          {icon}
          {hasDot && (
            <div className={`absolute -top-1 -right-1 w-2 h-2 rounded-full bg-[#5c6ad2] border-2 border-black transition-opacity duration-300 ${isCollapsed ? 'opacity-100' : 'opacity-0'}`}></div>
          )}
        </div>
        ...
        {isCollapsed && (
          <div className="absolute left-14 bg-[#1a1a1a] text-white text-xs px-2.5 py-1.5 rounded-md opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 font-medium border border-gray-800 shadow-xl transition-opacity">
            {label}
          </div>
        )}
      </button>
...
export default function Sidebar({ isMobileOpen, setIsMobileOpen }: { isMobileOpen?: boolean, setIsMobileOpen?: (val: boolean) => void }) {
  ...
  return (
    <>
      ...
      <div className={`h-screen bg-[#000000] text-white border-r border-[#1a1a1a] z-50 font-sans transition-all duration-300 ease-in-out overflow-hidden fixed md:relative w-[250px] ${isCollapsed ? 'md:w-[68px]' : 'md:w-[250px]'} ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex items-center justify-between pl-[22px] pr-4 pt-5 pb-4">
          ...
          <button
            ...
            className={`text-gray-500 hover:text-white transition-opacity duration-300 p-1 rounded-md hover:bg-[#1a1a1a] hidden md:block ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          >
        ...
        <div className="mt-auto relative pb-4 pt-2">
          <div className={`absolute bottom-[72px] left-[16px] transition-opacity duration-300 hidden md:block ${isCollapsed ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button
              ...
              className="text-gray-500 hover:text-white transition-colors p-2 rounded-full hover:bg-[#1a1a1a]"
            >
          ...
          <div className="pl-[16px]">
            {user ? (
              <div className="w-9 h-9 rounded-full bg-[#f43f5e] flex items-center justify-center text-white font-bold text-[14px] cursor-pointer hover:opacity-90 transition-opacity flex-shrink-0 shadow-sm">
                {user.displayName?.charAt(0)?.toUpperCase() || user.email?.charAt(0)?.toUpperCase() || 'J'}
              </div>
            ) : (
              <button onClick={loginWithGoogle} className="w-9 h-9 rounded-full bg-[#f43f5e] flex items-center justify-center text-white hover:opacity-90 transition-opacity flex-shrink-0 shadow-sm">
                <LogIn size={16} />
              </button>
            )}
          </div>
